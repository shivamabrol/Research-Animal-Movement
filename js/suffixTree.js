...
class SuffixTree {
  ...
  search(pattern) {
    ...
    while (curNode != null) {
      ...
      for (
        ;
        textIndex <= Math.min(selectedTrans[2], this.text.length - 1);
        textIndex++
      ) {
    ...
    return Array.from(matchedWordIds)
      ...
      .map((matchedWordId) =>
        this.words[matchedWordId].slice(
          0,
          this.words[matchedWordId].indexOf("#")
        )
      );
  }
      ...
      for (
        ...
        if (this.text[textIndex] === "#") {
          if (curElement.start < 0) {
            ...
            var matchedWordId = +this.text.slice(
              curElement.start - curElement.prefixCount,
              curElement.end
            );
      ...
      if (curElement.end < 0) {
        for (var key in curElement.transition[0].transitions) {
          frontier.push({
            ...
            prefixCount:
              curElement.start >= 0
                ? curElement.prefixCount +
                  Math.min(curElement.transition[2], this.text.length - 1) -
                  curElement.start +
                  1
                : 0,
